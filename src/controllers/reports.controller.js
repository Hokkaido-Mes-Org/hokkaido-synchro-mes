    ...
    try {
        ...
        if (tbody) {
            tbody.innerHTML = dados.map(d => {
                ...
                return `<tr class="border-b border-gray-100 hover:bg-gray-50"><td class="p-2 text-sm">${dataFmt}</td><td class="p-2 text-sm font-medium">${d._machine || '-'}</td><td class="p-2 text-sm">${d._shift || '-'}</td><td class="p-2 text-sm">${d._orderNumber}</td><td class="p-2 text-sm">${d._productCode}</td><td class="p-2 text-sm truncate max-w-xs" title="${d._productName}">${d._productName || '-'}</td><td class="p-2 text-sm text-right font-medium">${d._qty.toLocaleString('pt-BR')}</td><td class="p-2 text-sm text-right ${d._scrap > 0 ? 'text-red-600 font-medium' : ''}">${d._scrap.toLocaleString('pt-BR')}</td></tr>`;
            }).join('');
        }
    } catch (error) {
...
async function relBuscarPerdas() {
    ...
    try {
        ...
        if (tbody) {
            tbody.innerHTML = dados.map(d => {
                const dataFmt = d._date ? d._date.split('-').reverse().join('/') : '-';
                const perda = d._scrap > 0 ? d._scrap : d._scrapKg;
                const unit = d._scrap > 0 ? '' : ' kg';
                return `<tr class="border-b border-gray-100 hover:bg-gray-50"><td class="p-2 text-sm">${dataFmt}</td><td class="p-2 text-sm font-medium">${d._machine || '-'}</td><td class="p-2 text-sm">${d._shift || '-'}</td><td class="p-2 text-sm">${d._orderNumber}</td><td class="p-2 text-sm">${d._productCode}</td><td class="p-2 text-sm truncate max-w-xs">${d._productName || '-'}</td><td class="p-2 text-sm text-right text-red-600 font-bold">${perda.toLocaleString('pt-BR')}${unit}</td><td class="p-2 text-sm text-gray-500">${d._lossReason || '-'}</td></tr>`;
            }).join('');
        }
    } catch (error) {
...
function relExportarProducaoCSV() {
    ...
    const headers = ['Data', 'Máquina', 'Turno', 'OP', 'Código', 'Produto', 'Quantidade', 'Perdas'];
    const rows = relProducaoDados.map(d => [d._date, d._machine, d._shift, d._orderNumber, d._productCode, `"${d._productName}"`, d._qty, d._scrap]);
...
function relExportarPerdasCSV() {
    ...
    const headers = ['Data', 'Máquina', 'Turno', 'OP', 'Código', 'Produto', 'Perdas', 'Motivo'];
    const rows = relPerdasDados.map(d => [d._date, d._machine, d._shift, d._orderNumber, d._productCode, `"${d._productName}"`, d._scrap, `"${d._lossReason || ''}"`]);
...
function relExportarProducaoTabela() {
    if (relProducaoDados.length === 0) { notify('Nenhum dado para exportar', 'warning'); return; }
    let html = '<table><thead><tr><th>Data</th><th>Máquina</th><th>Turno</th><th>OP</th><th>Código</th><th>Produto</th><th>Qtd</th><th>Perdas</th></tr></thead><tbody>';
    relProducaoDados.forEach(d => { html += `<tr><td>${d._date}</td><td>${d._machine}</td><td>${d._shift}</td><td>${d._orderNumber}</td><td>${d._productCode}</td><td>${d._productName}</td><td style="text-align:right">${d._qty.toLocaleString('pt-BR')}</td><td style="text-align:right">${d._scrap.toLocaleString('pt-BR')}</td></tr>`; });
    html += '</tbody></table>';
...
function relExportarPerdasTabela() {
    if (relPerdasDados.length === 0) { notify('Nenhum dado para exportar', 'warning'); return; }
    let html = '<table><thead><tr><th>Data</th><th>Máquina</th><th>Turno</th><th>OP</th><th>Código</th><th>Produto</th><th>Perdas</th><th>Motivo</th></tr></thead><tbody>';
    relPerdasDados.forEach(d => { html += `<tr><td>${d._date}</td><td>${d._machine}</td><td>${d._shift}</td><td>${d._orderNumber}</td><td>${d._productCode}</td><td>${d._productName}</td><td style="text-align:right;color:red">${d._scrap.toLocaleString('pt-BR')}</td><td>${d._lossReason || '-'}</td></tr>`; });
    html += '</tbody></table>';
